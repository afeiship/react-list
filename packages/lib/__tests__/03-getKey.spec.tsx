...
describe('getKey', () => {
  ...
  it('should extract key using a function', () => {
    const extractor = ({ item }: { item: User }) => item.id;
    expect(getKey(users[0], 0, users, extractor)).toBe(1);
  });
  ...
  it('should pass index to function extractor', () => {
    const extractor = ({ index }: { index: number }) => index * 10;
    expect(getKey(users[1], 1, users, extractor)).toBe(10);
  });

  it('should pass data to function extractor', () => {
    const extractor = ({ data }: { data: User[] }) => data.length;
    expect(getKey(users[1], 1, users, extractor)).toBe(3);
  });
